...
// TimebasedMasterChef is a "semifredo of popsicle stand" this contract is created to provide single side farm for IFO of Popsicle Finance.
...
contract TimebasedMasterChef is Ownable {
    using SafeERC20 for IERC20;
    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many LP tokens the user has provided.
        uint256 rewardDebt; // Reward debt. See explanation below.
        uint256 remainingRewardTokenTokenReward;  // REWARDTOKEN Tokens that weren't distributed for user per pool.
        //
        ...
        //   1. The pool's `accREWARDTOKENPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint32 lastRewardTime; // Last timestamp number that REWARDTOKEN distribution occurs.
        uint16 allocPoint; // How many allocation points assigned to this pool. REWARDTOKEN to distribute per second.
        
        
    }
    
    IERC20 immutable public rewardtoken; // The REWARDTOKEN TOKEN!!
    
    uint256 public rewardtokenPerSecond; // RewardToken tokens vested per second.
    ...
    uint32 immutable public startTime; // The timestamp when REWARDTOKEN farming starts.
    ...
    // Changes RewardToken token reward per second. Use this function to moderate the `lockup amount`. Essentially this function changes the amount of the reward
    // which is entitled to the user for his token staking by the time the `endTime` is passed.
    //Good practrewardtoken to update pools without messing up the contract
...
// How many pools are in the contract
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.stakingTokenTotalAmount != 0) {
            ...
            accRewardTokenPerShare += rewardtokenReward * 1e12 / pool.stakingTokenTotalAmount;
        }
        return user.amount * accRewardTokenPerShare / 1e12 - user.rewardDebt + user.remainingRewardTokenTokenReward;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardtokenReward =
            multiplier * rewardtokenPerSecond * pool.allocPoint / totalAllocPoint;
        pool.accRewardTokenPerShare += rewardtokenReward * 1e12 / pool.stakingTokenTotalAmount;
        pool.lastRewardTime = uint32(block.timestamp);
    }

    // Deposit staking tokens to TimebasedMasterChef for REWARDTOKEN allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount * pool.accRewardTokenPerShare / 1e12 - user.rewardDebt + user.remainingRewardTokenTokenReward;
            user.remainingRewardTokenTokenReward = safeRewardTransfer(msg.sender, pending);
        }
    ...
    // Withdraw staked tokens from TimebasedMasterChef.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "TimebasedMasterChef: you cant eat that much popsicles");
        updatePool(_pid);
        uint256 pending =
            user.amount * pool.accRewardTokenPerShare / 1e12 - user.rewardDebt + user.remainingRewardTokenTokenReward;
        user.remainingRewardTokenTokenReward = safeRewardTransfer(msg.sender, pending);
    ...
    // Safe rewardtoken transfer function. Just in case if the pool does not have enough REWARDTOKEN token,
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns(uint256) {
        uint256 rewardtokenTokenBalance = rewardtoken.balanceOf(address(this));
        if (rewardtokenTokenBalance == 0) { //save some gas fee
            return _amount;
        }
        if (_amount > rewardtokenTokenBalance) { //save some gas fee
            rewardtoken.safeTransfer(_to, rewardtokenTokenBalance);
            return _amount - rewardtokenTokenBalance;
        }
